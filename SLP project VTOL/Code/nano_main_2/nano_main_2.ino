...
void setup() {
  // FIX 1: Drive pins LOW immediately on boot before servo library takes over.
  // This prevents the floating pin from sending a spurious high pulse to the ESC
  // during the reset/brownout window that occurs when a battery is disconnected.
  ...
  // FIX 2: Block here until we receive a valid SBUS signal (or timeout after 5s).
  // This prevents acting on garbage/stale data immediately after a brownout reset.
  // Motors are actively held at 1000us during the wait.
